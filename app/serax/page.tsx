...
import { SeraxProduct, getSeraxProducts, getSeraxCategories } from "@/sanity/lib/products/getSeraxProducts";
import ProductionImage from "@/components/ProductionImage";
import Header from "@/components/Header";

export default function SeraxPage() {
  const [products, setProducts] = useState<SeraxProduct[]>([]);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading Serax products...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {currentProducts.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No Serax products found.</p>
              <p className="text-gray-400 text-sm mt-2">
                Make sure Serax products are added to Sanity CMS and USE_SANITY_PRODUCTS=true is set.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {currentProducts.map((product) => (
                <Link
                  key={product._id}
                  href={`/serax/${product.slug?.current}`}
                  className="group"
                >
                    ...
                    <div className="relative aspect-square bg-gray-50">
                      {product.image?.asset?.url ? (
                        <ProductionImage
                          ...
                          alt={product.name || "Serax product"}
                          fill
