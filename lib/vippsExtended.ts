...
class VippsClientExtended {
  ...
  public async createCheckoutSession(sessionRequest: Record<string, unknown>): Promise<Record<string, unknown>> {
    await this.authenticate();
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Vipps create checkout session failed: ${response.status} ${errorText}`);
    }
