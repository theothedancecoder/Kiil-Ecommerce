...
async function fixTabTLED() {
  console.log('🎯 Fixing Tab T LED image...\n');

  const productInfo = {
    name: 'Tab T LED', 
    searchTerms: ['Tab T LED', 'Tab T', 'TAB T LED', 'TAB T'],
    imagePaths: [
  ...
  try {
    console.log(`🔍 Processing: ${productInfo.name}`);
        ...
        if (imageAsset && imageAsset.url) {
          ...
          try {
            ...
            if (response.ok) {
              ...
              return; // Skip if image is properly assigned and accessible
            } else {
    ...
    console.error('❌ Error fixing Tab T LED product:', error.message);
  }

  console.log('\n🎉 Tab T LED fix completed!');
}

fixTabTLED().catch(console.error);
